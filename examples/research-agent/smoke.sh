#!/usr/bin/env bash
# Smoke test for the research-agent example.
...
(cd "$HERE" && $HELIX start --port 18863 &)
PID=$!
sleep 3
if curl -fsS --max-time 3 http://127.0.0.1:18863/api/health >/dev/null 2>&1; then
  ok "runtime responds on :18863"
else
  echo "  ✗ runtime did not come up on :18863"
...
echo
echo "✅ research-agent smoke passed"
